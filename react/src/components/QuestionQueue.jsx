...
export default function QuestionQueue({
  ...
  const active = questions.filter((question) => !question.acknowledged);
  const done = questions.filter((question) => question.acknowledged);
    ...
    <div className="flex flex-col gap-3 h-full">
      ...
      <div className="flex flex-col gap-2 overflow-y-auto scrollbar-thin flex-1">
        {active.map((question) => (
          ...
          >
            {question.cluster_summaries && question.cluster_summaries.length > 0 ? (
              <div className="space-y-1.5">
                {question.cluster_summaries.map((summary) => (
                  <div key={`${question.question_id}-${summary.cluster_id}`} className="flex items-start gap-2">
                    <div className="flex items-center gap-1 mt-0.5">
                      <GroupsOutlinedIcon sx={{ fontSize: 13, color: "#64748b" }} />
                      <span className="text-xs text-slate-500">{summary.count}</span>
                    </div>
                    <p className="text-sm text-slate-700 flex-1">{summary.summary}</p>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-sm text-slate-700">
                {question.text || question.content || "Question received"}
              </p>
            )}
            ...
            <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-100">
              ...
              {onAcknowledge && (
                <button
                  onClick={() => onAcknowledge(question.question_id || question.id)}
        ...
        {done.length > 0 && (
          <>
            ...
            {done.map((question) => (
              <div key={question.question_id || question.id} className="card p-3 opacity-50">
